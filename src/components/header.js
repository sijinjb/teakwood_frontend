...
export default function Header() {
  ...
  return (
    <header className="inset-x-0 top-0 z-50 w-full fixed bg-[#0E6B66]">
      ...
      {/* Floating WhatsApp button + label with subtle animation */}
      <div className="fixed z-50 bottom-6 right-6 flex flex-col items-center">
        <a
          href="https://wa.me/918904088131"
          target="_blank"
          rel="noopener noreferrer"
          className="bg-[#24CC63] p-3 rounded-full text-white shadow-lg transform transition hover:scale-105 focus:outline-none animate-[float_3s_ease-in-out_infinite]"
          aria-label="WhatsApp"
          style={{
            // Fallback animation if Tailwind custom animate not available
...
<span className="mt-3 inline-block text-xs font-semibold text-white bg-gradient-to-r from-orange-300 to-amber-500 px-3 py-1 rounded-full shadow-md hover:scale-105 transition transform duration-300">
  Let’s Connect
</span>
      </div>

      {/* Inline keyframes for float animation (fallback) */}
      <style>{`
        @keyframes float {
